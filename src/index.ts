...
import PolyfactClientBuilder from "polyfire-js";

const pandoc = require("node-pandoc");

const polyfire = PolyfactClientBuilder({
  project: process.env.POLYFIRE_PROJECT_ID || "",
});

const {
  models: { transcribe, generate },
} = polyfire;

async function mdToHTML(md: string): Promise<string> {
  return new Promise((res, rej) =>
    pandoc(md, "-f markdown -t html -s", (err: Error, result: string) => {
      if (err) {
        rej(err);
      } else {
        res(result);
...
(async () => {
  ...
  if (!isAuthenticated) {
    await polyfire.auth.login({
      provider: "custom",
      token: process.env.POLYFIRE_TOKEN || "",
    });
  }
  ...
  bot.on("message", async (msg) => {
    ...
    try {
      ...
      if (msg.voice) {
        ...
        bot.sendMessage(
          chatId,
          "Got it. I will generate an article based on the audio you sent 🫡\nYou can expect it to take up to a few minutes ⏳"
        );
        ...
        bot.sendMessage(
          chatId,
          `Done 🔥\nHere's your article !\nI named it **${title}**\nNext is the markdown version along with a preview you can open in a browser.`,
          { parse_mode: "Markdown" }
        );
        bot.sendDocument(
          chatId,
          Buffer.from(transcription, "utf8"),
          {},
          { filename: `${slug}.md`, contentType: "text/markdown" }
        );
        bot.sendDocument(
          chatId,
          Buffer.from(await mdToHTML(transcription), "utf8"),
          {},
          { filename: `preview.html`, contentType: "text/html" }
        );
      }
    } catch (err) {
      console.error(err);
